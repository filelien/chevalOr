      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-2 lg:grid-cols-5">
        <div className="lg:col-span-2">
          <div className="flex items-center gap-3">
            <HotelLogo src={siteLogo ?? logo} size="sm" tone="dark" wrap adminKey="site.logo" />
          </div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-white/70">
            <li>{hotel.address}</li>
            {(hotel as any).locationHint && (
              <li>
                <span className="text-white/90">Localisation : </span>
                {(hotel as any).locationHint}
              </li>
            )}
            <li>
              <a
                href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${hotel.address} ${(hotel as any).locationHint ?? ""}`)}`}
                target="_blank"
            ...
            {(hotel as any).secondaryPhone && <li><a href={`tel:${(hotel as any).secondaryPhone}`} className="hover:text-gold">{(hotel as any).secondaryPhone}</a></li>}
