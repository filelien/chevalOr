...
export function NewsletterForm({ compact }: { compact?: boolean }) {
  ...
  return (
    <form onSubmit={submit} className={compact ? "flex gap-2" : "space-y-3"}>
      {!compact && <p className="text-sm text-white/60">{t.footer.newsletterDesc}</p>}
      <div className={compact ? "flex flex-1 gap-2" : "flex flex-col gap-2 sm:flex-row"}>
        <input
          type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
          placeholder="email@exemple.com"
          className="flex-1 rounded-md border border-white/20 bg-white/10 px-3 py-2 text-sm text-white placeholder:text-white/40"
        />
        <Button type="submit" variant="hero" size="sm" disabled={busy}>{busy ? "…" : t.footer.subscribe}</Button>
      </div>
    </form>
  );
}
