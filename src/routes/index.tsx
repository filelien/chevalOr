...
import { fetchEditableNewsletter } from "@/lib/site-editable-content";
...
function Index() {
  ...
  const { data: newsletter } = useQuery({ queryKey: ["public-newsletter-content"], queryFn: fetchEditableNewsletter });
      ...
      <section className="border-t border-border py-16">
        <div className="mx-auto max-w-xl px-6 text-center">
          <SectionHeader
            ...
            title={newsletter?.title || "Inspirations & offres privées"}
            subtitle={newsletter?.subtitle || "Rejoignez notre cercle — offres exclusives, événements, guides voyage."}
          />
          <div className="mt-8"><NewsletterForm buttonText={newsletter?.buttonText} tone="light" /></div>
        </div>
      </section>
    </SiteShell>
  );
