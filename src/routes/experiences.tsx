...
import { fetchEditableExperiences } from "@/lib/site-editable-content";
...
function ExperiencesPage() {
  ...
  const { data: editableExperiences } = useQuery({ queryKey: ["public-experiences-content"], queryFn: fetchEditableExperiences });
  const { t } = useI18n();
  const u = t.ui.experiences;
  const experiences = editableExperiences?.length ? editableExperiences : EXPERIENCES;
          ...
          {experiences.map((e) => (
            <Reveal key={e.title}>
              <div className="overflow-hidden rounded-xl bg-card shadow-elegant">
                <img src={imgs[e.image as keyof typeof imgs] || e.image || hero} alt={e.title} className="aspect-[16/10] w-full object-cover" />
                <div className="p-6">
