...
export function mergeHotelInfo(cms: Partial<HotelInfoCms> = {}) {
  return {
    ...
    locationHint: cms.locationHint ?? (HOTEL as any).locationHint ?? "",
    phone: cms.phone ?? HOTEL.phone,
    secondaryPhone: cms.secondaryPhone ?? (HOTEL as any).secondaryPhone ?? "",
    email: cms.email ?? HOTEL.email,
    whatsapp: cms.whatsapp ?? HOTEL.whatsapp,
    roomCount: cms.roomCount ?? (HOTEL as any).roomCount ?? 0,
